...
# user params
_AVS2YUVPathPE='C:/encoding/avs2yuv.exe'
_X264PathPE='C:/encoding/x264/x264-10bit.exe'
_FFMS210bitPathPE='C:/encoding/oldplugins/ffms2-10bit.dll'

#_X264ExtraParameters='--ref 1 --no-deblock --no-cabac --subme 1 --partitions none --me dia --aq-mode 0 --no-mbtree'
_X264ExtraParameters='--preset ultrafast --subme 1 --input-depth 16'
...
def JoinedAviSynthScript(_OutputAviSynthScript,_LosslessFile,_AviSynthMemoryPerThread,_TotalThreads):
  ...
  _JAS.write('LoadPlugin("' + os.path.normpath(_FFMS210bitPathPE) + '")\n')
...
def CountAviSynthFrames(AviSynthScript, proc):
  ...
  os.close(a)
  CmdLineFrames = '"' + os.path.normpath(_AVS2YUVPathPE) + '" -raw -frames 1 "[input]" -o "' + tempYUV + '"'
  if(options.usewine):
...
parser.add_option('--threads', type='int', dest='TotalThreads', default=2)
parser.add_option('--max-memory', type='int', dest='AviSynthMemoryPerThread', default=512)
parser.add_option('--wine', action='store_true', dest='usewine', default=False)
parser.add_option('--avs2yuv', action='store_true', dest='useavs2yuv', default=True)
(options, args) = parser.parse_args()

if(options.usewine):
  options.useavs2yuv = True
_TotalThreads = options.TotalThreads
_AviSynthMemoryPerThread = options.AviSynthMemoryPerThread
...
for file in args:
  ...
  if(options.useavs2yuv):
    _CmdLinePE = _CmdLinePE + '"' + os.path.normpath(_AVS2YUVPathPE) + '" -raw ' + _CmdLineInput + ' -o - | '
